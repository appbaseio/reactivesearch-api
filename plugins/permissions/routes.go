	...
	var routes = []plugin.Route{
		{
			...
			HandlerFunc: opClassifier(basicAuth(validateOp(validateACL(p.getPermission())))),
		...
		},
		{
			...
			Path:        "/permission",
			HandlerFunc: opClassifier(basicAuth(validateOp(validateACL(p.putPermission())))),
		...
		},
		{
			...
			Path:        "/permission/{username}",
			HandlerFunc: opClassifier(basicAuth(validateOp(validateACL(p.patchPermission())))),
		...
		},
		{
			...
			Path:        "/permission/{username}",
			HandlerFunc: opClassifier(basicAuth(validateOp(validateACL(p.deletePermission())))),
