...
type mSearchResponseEs7 struct {
	Responses []searchResponseEs7 `json:"responses"`
}

func (a *Analytics) recordAnalyticsES7(esResponse searchResponseEs7, docID, searchID string, isUsingExistingSearchID bool, isRSAPI bool, rsRequestBody *querytranslate.RSQuery, r *http.Request, rules *[]string) {
	...
	record.TotalHits = &esResponse.Hits.Total.Value
...
func (a *Analytics) recordResponseEs7(docID, searchID string, isUsingExistingSearchID bool, isRSAPI bool, rsRequestBody *querytranslate.RSQuery, r *http.Request, responseBody []byte) {
	var esResponse searchResponseEs7
	if isRSAPI {
		took, err := jsonparser.GetFloat(responseBody, "settings", "took")
		if err != nil {
			log.Warnln(logTag, "unable to parse took value", err)
			// TODO: took value is expected for analytics recording
			took = 64.64
		}
		var total int64
		err2 := jsonparser.ObjectEach(responseBody, func(key []byte, value []byte, dataType jsonparser.ValueType, offset int) error {
			// handle suggestions type of queries
			if !contains(querytranslate.RESERVED_KEYS_IN_RESPONSE, string(key)) {
				...
				if queryType != nil && *queryType == querytranslate.Suggestion {
					...
					totalHits, err := jsonparser.GetInt(value, "hits", "total", "value")
					if err != nil {
		...
		esResponse.Hits.Total.Value = total
		var rules []string
		...
		a.recordAnalyticsES7(esResponse, docID, searchID, isUsingExistingSearchID, isRSAPI, rsRequestBody, r, &rules)
		return
	} else {
		if strings.Contains(r.RequestURI, "_msearch") {
			var m mSearchResponseEs7
		...
		a.recordAnalyticsES7(esResponse, docID, searchID, isUsingExistingSearchID, isRSAPI, rsRequestBody, r, nil)
		return
	}
}
