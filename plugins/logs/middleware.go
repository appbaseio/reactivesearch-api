...
func (l *Logs) recorder(h http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Store the `disable-logs` flag in the context.
		disableLogsCtx := difference.NewContext(r.Context(), &l.disableLogging)
		r = r.WithContext(disableLogsCtx)

		// Skip if logging is disabled
		if l.disableLogging {
			log.Debug(logTag, ": Logging is disabled, skipping!")
			h(w, r)
			return
		}
...
func (l *Logs) recordResponse(w *httptest.ResponseRecorder, r *http.Request, reqBody []byte) {
	...
	startAsPtr, fetchErr := tracktime.FromTimeTrackerContext(r.Context())
	if fetchErr != nil {
		log.Warnln(logTag, ": error while fetching time from ctx to calculate time taken: ", fetchErr.Error())
	} else {
		rec.Took = int(time.Since(*startAsPtr).Milliseconds())
	}
...
func (l *Logs) captureStageChanges(rec record, requestChangesByStage map[string]RequestChange, responseChangesByStage map[string]RequestChange) record {
	...
	for stage, changes := range requestChangesByStage {
		...
		timeTaken := changes.Before.TimeTaken + changes.After.TimeTaken
