...
// IsDiffingDisabled indicates whether or not diffing should be done
func (l *Logs) IsDiffingDisabled() bool {
	return l.disableLogging || !l.enableDiffing
}
...
func (l *Logs) InitFunc() error {
	...
	// Set the value for disableLogging
	//
	// We will read the value from an environment variable
	// through it is passed as a commandline flag.
	shouldDisableLogging := os.Getenv("SHOULD_DISABLE_LOGGING")
	if shouldDisableLogging == "true" {
		l.disableLogging = true
	} else {
		l.disableLogging = false
	}
	log.Debug(logTag, ": Logging is disabled: ", l.disableLogging)
