	...
	var routes = []plugin.Route{
		{
			...
			Path:        "/user",
			HandlerFunc: basicAuth(opClassifier(validateOp(validateACL(u.getUser())))),
		...
		},
		{
			...
			Path:        "/user",
			HandlerFunc: basicAuth(opClassifier(validateOp(validateACL(u.putUser())))),
		...
		},
		{
			...
			Path:        "/user",
			HandlerFunc: basicAuth(opClassifier(validateOp(validateACL(u.patchUser())))),
		...
		},
		{
			...
			Path:        "/user",
			HandlerFunc: basicAuth(opClassifier(validateOp(validateACL(u.deleteUser())))),
