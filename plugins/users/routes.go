	...
	var routes = []plugin.Route{
		{
			...
			Path:        "/user",
			HandlerFunc: opClassifier(basicAuth(validateOp(validateACL(u.getUser())))),
		...
		},
		{
			...
			Path:        "/user",
			HandlerFunc: opClassifier(basicAuth(validateOp(validateACL(u.putUser())))),
		...
		},
		{
			...
			Path:        "/user",
			HandlerFunc: opClassifier(basicAuth(validateOp(validateACL(u.patchUser())))),
		...
		},
		{
			...
			Path:        "/user",
			HandlerFunc: opClassifier(basicAuth(validateOp(validateACL(u.deleteUser())))),
