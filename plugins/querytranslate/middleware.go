...
func queryTranslate(h http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, req *http.Request) {
		...
		reqPermission, _ := permission.FromContext(req.Context())
		...
		var mSearchDetails *[]MSearchDetails
		msearchQuery, _, mSearchDetails, translateErr = translateQuery(*body, iplookup.FromRequest(req), nil, preference)
		...
		// Inject the msearch details into context
		ctxWithMSearchDetails := NewMSearchDetailsRequestContext(req.Context(), *mSearchDetails)
		req = req.WithContext(ctxWithMSearchDetails)
		...
		log.Debugln(logTag, "should log diff: ", shouldLogDiff)
		log.Debugln(logTag, "is logs disabled: ", *isLogsDisabled)
		if shouldLogDiff && !*isLogsDisabled {
			if rl != nil {
				rl.LogsDiffing.Add(1)
