...
func (o *CompoundClauseType) UnmarshalJSON(bytes []byte) error {
	...
	err := json.Unmarshal(bytes, &clauseType)
	if err != nil {
		return err
	}
	switch clauseType {
	...
	case Filter.String():
		*o = Filter
	default:
		return fmt.Errorf("invalid suggestion type encountered: %v", clauseType)
	}
	return nil
}
...
func (o CompoundClauseType) MarshalJSON() ([]byte, error) {
	...
	switch o {
	case Must:
		clauseType = Must.String()
	case Filter:
		clauseType = Filter.String()
	default:
		return nil, fmt.Errorf("invalid suggestion type encountered: %v", o)
	}

	return json.Marshal(clauseType)
}
...
func (o SuggestionType) MarshalJSON() ([]byte, error) {
	...
	case FAQ:
		suggestionType = FAQ.String()
	case Document:
		suggestionType = Document.String()
	default:
		return nil, fmt.Errorf("invalid suggestion type encountered: %v", o)
	}
...
// AIConfig will contain details about the AnswerAI part of
// the query
...
type Query struct {
	...
	Type                        QueryType                         `json:"type" jsonschema:"title=type,description=type of query" jsonschema_extras:"engine=elasticsearch,engine=mongodb,engine=solr,engine=opensearch"`
	...
	EnableFAQSuggestions        *bool                             `json:"enableFAQSuggestions,omitempty" jsonschema:"title=enableFAQSuggestions,description=whether or not to enable FAQ suggestions" jsonschema_extras:"engine=elasticsearch,engine=opensearch"`
	FAQSuggestionsConfig        *FAQSuggestionsOptions            `json:"FAQSuggestionsConfig,omitempty" jsonschema:"title=FAQSuggestionsConfig,description=additional options for getting FAQ suggestions" jsonschema_extras:"engine=elasticsearch,engine=opensearch"`
	...
	FAQSearchBoxId              *string                           `json:"faqSearchboxId,omitempty" jsonschema:"title=faqSearchboxId,description=searchbox id for a faq suggestion query, uses searchboxId if not specified" jsonschema_extras:"engine=elasticsearch,engine=opensearch"`
	...
	EnableAI                    *bool                             `json:"enableAI,omitempty" jsonschema:"title=enableAI,description=Whether or not to enable Answer AI to inject AI based answer in the response" jsonschema_extras:"engine=opensearch"`
	AIConfig                    *AIConfig                         `json:"AIConfig,omitempty" jsonschema:"title=AI Config,description=Configuration for the AnswerAI part of the query. This will only be considered if 'enableAI' is set to 'true'" jsonschema_extras:"engine=opensearch"`
	CompoundClause              *CompoundClauseType               `json:"compoundClause,omitempty" jsonschema:"title=Compound Clause,description=Configure whether the DSL query is generated with the compound clause of 'must' (default for 'search' and 'geo') or 'filter' (default for 'range' and 'term'). Setting the compound clause to filter allows search engine to cache and allows for higher throughput in cases where scoring isn't relevant (e.g. term, geo or range type of queries that act as filters on the data)" jsonschema_extras:"engine=opensearch,engine=elasticsearch"`
	EnableDocumentSuggestions   *bool                             `json:"enableDocumentSuggestions,omitempty" jsonschema:"title=enableDocumentSuggestions,description=whether or not to enable document suggestions" jsonschema_extras:"engine=elasticsearch,engine=opensearch"`
	DocumentSuggestionsOptions  *RecentDocumentSuggestionsOptions `json:"documentSuggestionsConfig,omitempty" jsonschema:"title=documentSuggestionsConfig,description=additional options to specify for document suggestions" jsonschema_extras:"engine=elasticsearch,engine=opensearch"`
}
...
func (query *Query) getQuery(rsQuery RSQuery) (*interface{}, map[string]interface{}, bool, error) {
	...
	if len(finalQuery) != 0 {
		...
		} else if (query.Type == Search && !shouldApplyKnn(*query)) || query.Type == Suggestion {
			// If it is a search query and kNN needs to be applied, we don't
			// want the query to be built.
	...
	// Don't build the query itself if it will be a kNN query
	if !shouldApplyKnn(*query) {
		queryByType, err := query.generateQueryByType()
		return queryByType, finalOptions, true, err
	}

	var defaultQuery interface{} = map[string]interface{}{}
	return &defaultQuery, finalOptions, true, nil
}
...
func (query *Query) applyCustomQuery(byTypeFunc QueryByType) (*interface{}, map[string]interface{}, error) {
	...
	// If the `originalQuery` is nil, return as is.
	// Marshal the originalQuery and see if it returns a nil value
	marshalledQuery, marshalErr := json.Marshal(originalQuery)
	if marshalErr != nil || string(marshalledQuery) == "null" {
		var nilQuery interface{} = nil
		log.Debug(logTag, ": returning since original query is nil")
		return &nilQuery, queryOptions, nil
	}

	// Add the compound clause
	if query.CompoundClause == nil {
		defaultClause := GetDefaultClauseBasedOnType(query.Type)
		query.CompoundClause = &defaultClause
	}

	var queryWithClause interface{}
	queryWithClause = map[string]interface{}{
		"bool": map[string]interface{}{
			query.CompoundClause.String(): originalQuery,
		},
	}

	return &queryWithClause, queryOptions, nil
}
...
func (query *Query) shouldExecuteQuery() bool {
	// Give the highest priority to the `execute` field
	//
	// If it is explicitly `false`, we return right away.
	if query.Execute != nil && !*query.Execute {
		return false
	}

	// If the query is of type suggestion, then if either of
	// index or endpoint is enabled, we return `true`.
	if query.Type == Suggestion &&
		((query.EnableIndexSuggestions == nil || *query.EnableIndexSuggestions) ||
			(query.Endpoint != nil && query.EnableEndpointSuggestions != nil && *query.EnableEndpointSuggestions)) {
		return true
	} else if query.Type == Suggestion {
		// For any other type of suggestion, we don't "execute"
		// because suggestions middleware will take care of injecting those
		// results.
		return false
	}
...
// MakeDataFieldReadable will convert a dataField to make it
// more readable.
//
// `some_title` will be converted to `Some Title`
func MakeDataFieldReadable(df string) string {
	// Replace all the underscores in the string with a space
	df = strings.Replace(df, "_", " ", -1)
	return strings.Title(df)
}

// GenerateHashFromBytes will generate a hash of the passed
// bytes string and return it accordingly.
func GenerateHashFromBytes(fromBytes []byte) string {
	hash := md5.Sum(fromBytes)
	return fmt.Sprintf("%x", hash)
}

// PreparedQuery is the query prepared that will
// be converted to msearch.
...
// MSearchDetails will contain details about the msearch
// query
