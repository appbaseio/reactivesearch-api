...
// ClusterID represents the (decrypted) cluster id for reactivesearch.io cloud and byoc clusters
...
func HTTPClient() *http.Client {
	once.Do(func() {
		...
		var netClient = &http.Client{
			Timeout:   time.Second * time.Duration(GetHTTPTimeout()),
			Transport: netTransport,
		}
		client = netClient
	})

	// If tracing is enabled, we will need to return it with
	// tracing enabled.
	if !GetTracingEnabled() {
		return client
	}

	return httptrace.WrapClient(client, httptrace.RTWithServiceName(GetServiceName()))
}

// getHTTPTimeout gets HTTP timeout from env variable
...
// GetNestedValueFromContext iterates the passed context
// until either the value is not found, not a map or the
// value is found.
func GetNestedValueFromContext(nestedKeys string, context map[string]interface{}) (interface{}, error) {
	nestedKeysArr := strings.Split(nestedKeys, ".")
	contextToFetchFrom := context

	for indexOfKey, keyValue := range nestedKeysArr {
		valueMapped, ok := contextToFetchFrom[keyValue]
		if !ok {
			// Make the error more meaningful.
			//
			// If the key passed is `someObj.someObj2.someKey`
			// and `someKey` is not found, we need to show
			// error accordingly.
			notFoundWhere := strings.Join(nestedKeysArr[0:indexOfKey], ".")

			// In case the key is top level
			if notFoundWhere == "" {
				notFoundWhere = "context or envs"
			}

			return nil, fmt.Errorf("no `%s` key present inside `%s`", keyValue, notFoundWhere)
		}

		// If the key is the last one, we need to get
		// return it accordingly.
		if indexOfKey == len(nestedKeysArr)-1 {
			// It is the last key
			return valueMapped, nil
		}

		// It is not the last key
		// Get the valueMapped from the key and then convert
		// it into a map.
		valueMappedAsMap, mapOk := valueMapped.(map[string]interface{})
		if !mapOk {
			return nil, fmt.Errorf("`%s` is not an object, incorrect syntax passed for substitution!", keyValue)
		}

		contextToFetchFrom = valueMappedAsMap
	}

	return nil, nil
}

// IsSSEEndpoint will check if the passed endpoint is an SSE
// endpoint and accordingly return `true` or `false`.
func IsSSEEndpoint(req *http.Request) bool {
	isMatch, _ := regexp.MatchString("^/_ai/.*/sse$", req.RequestURI)

	return (req.Method == http.MethodGet || req.Method == http.MethodPost) && isMatch
}
